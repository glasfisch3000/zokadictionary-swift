...
struct ReferencesGet: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "get",
        abstract: "Read a word reference stored in the database.",
//        usage: <#T##String?#>,
//        discussion: <#T##String#>,
        version: "0.0.0",
        shouldDisplay: true,
    ...
    func run() async throws {
        ...
        let reference = try await Reference.find(referenceID, on: app.db)
        try await reference?.$source.load(on: app.db)
        try await reference?.$destination.load(on: app.db)
        
        try await app.asyncShutdown()
        
        if let reference = reference?.toDTO() {
            switch outputFormat {
            case .json:
            ...
            case .yaml:
                ...
                let string = try encoder.encode(reference)
                print(string)
            }
        } else {
            print("reference not found for id \(referenceID)")
        }
    }
}
