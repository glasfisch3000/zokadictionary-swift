...
struct WordsDelete: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "delete",
        abstract: "Delete a word stored in the database.",
//        usage: <#T##String?#>,
//        discussion: <#T##String#>,
        version: "0.0.0",
        shouldDisplay: true,
    ...
    func run() async throws {
        ...
        if let word = try await Word.find(wordID, on: app.db) {
            try await word.delete(on: app.db)
            print("deleted word \(wordID)")
        } else {
