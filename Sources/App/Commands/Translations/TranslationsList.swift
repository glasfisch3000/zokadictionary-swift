...
struct TranslationsList: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "list",
        abstract: "List translations stored in the database.",
//        usage: <#T##String?#>,
//        discussion: <#T##String#>,
        version: "0.0.0",
        shouldDisplay: true,
    ...
    func run() async throws {
        ...
        var query = Translation.query(on: app.db)
        
        if let word = word {
            query = query.filter(\.$word.$id == word)
        }
        
        if let search = search {
            query = query.filter(\.$translation =~ search)
        }
        
        let translations = try await query
        ...
        case .yaml:
            ...
            let string = try encoder.encode(translations)
            print(string)
        }
    }
}
